...
import { ToastController, LoadingController, Platform, Loading, AlertController, ModalController, Modal, Alert} from 'ionic-angular';
...
import { File, FileEntry } from '@ionic-native/file/index';
...
import { APP_DOWNLOAD, APK_DOWNLOAD, IMAGE_SIZE, QUALITY_SIZE } from "../constants/Constants";
...
export class NativeService {
  ...
  checkPermission() {
      if (this.isMobile()) {
          ...
          this.androidPermissions.checkPermission(this.androidPermissions.PERMISSION.INTERNET).then(success => {
              if (!success.hasPermission) {
                  this.androidPermissions.requestPermission(this.androidPermissions.PERMISSION.INTERNET).catch(err => this.log(err));
              }
          });
          this.androidPermissions.checkPermission(this.androidPermissions.PERMISSION.WRITE_EXTERNAL_STORAGE).then(success => {
              if (!success.hasPermission) {
                  this.androidPermissions.requestPermission(this.androidPermissions.PERMISSION.WRITE_EXTERNAL_STORAGE).catch(err => this.log(this.showObject(err)));
              }
          });
          this.androidPermissions.checkPermission(this.androidPermissions.PERMISSION.MOUNT_UNMOUNT_FILESYSTEMS).then(success => {
              if (!success.hasPermission) {
          ...
          this.androidPermissions.checkPermission(this.androidPermissions.PERMISSION.USE_FINGERPRINT).then(success => {
              if (!success.hasPermission) {
                  this.androidPermissions.requestPermission(this.androidPermissions.PERMISSION.USE_FINGERPRINT);
              }
          });
          this.androidPermissions.checkPermission(this.androidPermissions.PERMISSION.RECORD_AUDIO).then(success => {
              if (!success.hasPermission) {
  ...
  showObject(obj,time=2000): void
  {
      if (typeof (obj) == "object")
      {
          ...
          this.showToast(tmp, time);
      }
      else
      {
          this.showToast(obj, time);
      }
  }
  /**
*打印对象信息
*/
  Object2String(obj): string {
      if (typeof (obj) == "object") {
          let tmp = "";
          for (let i in obj) {
              var property = obj[i];
   ...
   * @param duration 显示时长
   */
  showToast(message: string = '操作完成', duration: number = 500): void {
    if (this.isMobile()) {
   ...
   * @param content 显示的内容
   */
  showLoading(content: string = 'please wait....'): void {
